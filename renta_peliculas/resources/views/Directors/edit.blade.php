...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-md">
            ...
            <form action="{{ route('directors.update', $directors->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="name" class="form-label">Nombre del actors</label>
                    <input type="text" class="form-control w-50 mx-auto" id="name" name="name" value="{{ $directors->name }}" required>
                </div>
                <div class="mb-3">
                    <label for="date_birth" class="form-label">Fecha de nacimiento</label>
                    <input type="date" class="form-control w-50 mx-auto" id="date_birth" name="date_birth" value="{{ $directors->date_birth }}" required>
                </div>
                <div class="mb-3">
                    <label for="nationality" class="form-label">Nacionalidad del actors</label>
                    <input type="text" class="form-control w-50 mx-auto" id="nationality" name="nationality" value="{{ $directors->nationality }}" required>
                </div>
                <div class="mb-3">
                    <label for="biography" class="form-label">Biografia</label>
                    <textarea class="form-control w-50 mx-auto" id="biography" name="biography">{{ $directors->biography }}</textarea>
                </div>
                <div class="mb-3">
                    ...
                    @if ($directors->image)
                        <div class="mt-3">
                            <img src="{{ asset('assets_director/' . $directors->image) }}" alt="Imagen del actors" class="img-thumbnail" style="max-width: 200px;">
                        </div>
                    @endif
                </div>
